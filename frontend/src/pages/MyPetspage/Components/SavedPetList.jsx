...
const SavedPetList = () => {
    ...
    if (!savedPets || savedPets.length === 0) {
        return (
            <>
                ...
                <div className="">
                    <Inner>
                        <Center><h3 style={{ padding: '25px 0' }}>No saved pets found. Check out the search page!</h3></Center>
    ...
    return (
        <>
            ...
            <ImageContainer>
                {savedPets.map((pet) => (
                    <PetSimple isSaved={true} key={pet._id} id={pet._id} name={pet.name} image={`/${pet.image}`} breed={pet.breed} status={pet.status} />
                ))}
            </ImageContainer>
        </>
    );
