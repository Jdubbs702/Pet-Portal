...
const Dropdown = ({ list, width, text, selectedItem, onSelect }) => {
    ...
    useEffect(() => {
        ...
        return () => { //triggers once on unmount
        ...
        <div className="Dropdown" style={width} >
            ...
            {isOpen && selectedItem &&
                <div className="list">
                    ...
                    {list.map(i => (
                        <div className={i.id === selectedItem.id ? 'selected' : ''} key={i.id} onClick={() => { selectHandler(i) }}>{i.value}</div>
                    ))}
                </div>
            }
            {(isOpen && !selectedItem) &&
                <div className="list">
                    <div onClick={clearSelection}>--clear selection--</div>
                    {list.map(i => (
                        <div key={i.id} onClick={() => { selectHandler(i) }}>{i.value}</div>
                    ))}
                </div>
            }
        </div>
